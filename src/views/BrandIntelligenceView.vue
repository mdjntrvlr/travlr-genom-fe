...
const steps: StepItem[] = [
  { number: 1, label: 'Brand Extraction', active: true },
...
const submitBrandExtraction = async () => {
  ...
  try {
    ...
    submitSuccess.value = 'Brand extraction request submitted successfully.'
  } catch (error) {
    submitError.value =
...
<template>
  ...
  <section class="mt-7 flex flex-wrap items-center gap-4">
    <template v-for="(step, index) in steps" :key="step.label">
      <div class="flex items-center gap-3">
        <span
          :class="[
            ...
            step.active ? 'bg-cyan-500 text-white' : 'bg-slate-400 text-white',
          ]"
        >
          {{ step.number }}
        </span>
        <span
          :class="[
            'text-[18px]',
            step.active ? 'font-medium text-slate-900' : 'font-normal text-slate-500',
          ]"
        >
          {{ step.label }}
        </span>
      ...
      <div
        ...
        class="hidden h-1 w-[110px] rounded-full bg-slate-300 xl:block"
      ></div>
    </template>
  </section>
  ...
  <section class="mt-8 max-w-[1115px] rounded-2xl bg-slate-100 px-6 py-6">
    ...
    <div class="mt-6">
      ...
      <p v-if="websiteUrlError" class="mt-2 max-w-[820px] text-[12px] font-medium text-rose-600">
        {{ websiteUrlError }}
      </p>
    ...
    >
      {{ isSubmitting ? 'Submitting...' : 'Extract Brand' }}
    </button>

    <p v-if="submitError" class="mt-3 text-[12px] font-medium text-rose-600">
      {{ submitError }}
    </p>
    <p v-if="submitSuccess" class="mt-3 text-[12px] font-medium text-emerald-600">
      {{ submitSuccess }}
    </p>
  </section>
